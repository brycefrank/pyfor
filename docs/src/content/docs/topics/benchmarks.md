---
title: Benchmarks
description: pyfor, lidR, and PDAL compared on a single tile, and what the comparison revealed.
---

How does pyfor compare to other tools that process LiDAR for forest inventory? This page reports a
benchmark of pyfor, [lidR](https://github.com/r-lidar/lidR), and [PDAL](https://pdal.org) on a
single tile, and what the comparison revealed about pyfor.
...
The tile is the one that ships with pyfor, `pyfortest/data/test.laz`: 217,222 points over a 200 m by
200 m area, a 728 KiB `.laz`.
...
| `normalize` | `Cloud.normalize(1)`, the Zhang et al. (2003) progressive morphological filter | `normalize_height(las, tin())`, a Delaunay triangulation of the ground points | `filters.smrf` then `filters.hag_nn` |
| `chm` | `grid(1).raster("max", "z")` written to GeoTIFF | `rasterize_canopy(las, 1, p2r())` | `writers.gdal` with `output_type: max` |
| `chm_normalized` | normalize, then the `chm` operation | `normalize_height(las, tin())`, then the `chm` operation | `filters.smrf`, `filters.hag_nn`, then `writers.gdal` on `HeightAboveGround` |
| `metrics` | `normalize(1)` then `grid(20).standard_metrics(2)`, 25 rasters | `normalize_height(las, tin())` then `pixel_metrics(stdmetrics, res = 20)` | not supported: PDAL has no area based metrics stage |
...
* Every measurement is the wall clock of a fresh process (`Rscript`, `python`, `pdal`), so package
  loading is included. The `in-process` column is the time from after the interpreter and libraries
  are loaded to the end of the operation, as reported by the tool itself, which is what the
...
* PDAL's raster grid is pinned to the grid pyfor produces (`origin_x`, `origin_y`, `width`,
  `height`). Both tools now use the same origin convention, so the pinning states which grid to use
  rather than papering over two different ones; the verification below is what shows the two agree
  cell for cell.
...
| operation | tool | wall clock | in-process |
| --- | --- | --- | --- |
| `read` | pyfor | 1.644 | 0.071 |
| `read` | lidR | 5.251 | 0.354 |
| `read` | pdal | 0.219 | -- |
| `readwrite` | pyfor | 1.751 | 0.140 |
| `readwrite` | lidR | 5.309 | 0.440 |
| `readwrite` | pdal | 0.318 | -- |
| `normalize` | pyfor | 2.297 | 0.740 |
| `normalize` | lidR | 6.351 | 1.453 |
| `normalize` | pdal | 0.654 | -- |
| `chm` | pyfor | 1.723 | 0.140 |
| `chm` | lidR | 5.506 | 0.510 |
| `chm` | pdal | 0.239 | -- |
| `chm_normalized` | pyfor | 2.334 | 0.736 |
| `chm_normalized` | lidR | 6.498 | 1.702 |
| `chm_normalized` | pdal | 0.775 | -- |
| `metrics` | pyfor | 3.025 | 1.409 |
| `metrics` | lidR | 7.001 | 2.025 |

Reading the numbers:

* On a tile this size the wall clock is dominated by starting the process and loading libraries, not
  by the work. pyfor spends about 1.6 s of its 1.72 s `chm` run on `import pyfor`, which pulls in
  geopandas, rasterio, matplotlib, and laspy. lidR spends about 5.0 s of its 5.51 s loading R and
...
3. **A grid can be stated explicitly.**
   [GridSpec](/pyfor/api/pyfor.rasterizer/#pyfor.rasterizer.GridSpec) is an origin, a cell size, and
   a size in cells, and [Cloud.grid](/pyfor/api/pyfor.cloud/#pyfor.cloud.Cloud.grid),
   ...
   builds one that covers a whole collection, and
   [Retiler.retile_raster](/pyfor/api/pyfor.collection/#pyfor.collection.Retiler.retile_raster)
   snaps its tiling origin to the cell size.
4. **Trimming a raster refuses to guess.**
   ...
   round a bounding box to the nearest cell, which silently labelled an array with a grid it was not
   on. It now raises when the box does not fall on the cells.
